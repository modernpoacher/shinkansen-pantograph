...
function graphite(_ref10) {
  ...
  switch (type) {
    case OMEGA.QUERY:
      // console.log(`Pantograph.graphite():${OMEGA.QUERY}`, { type }) // eslint-disable-line
      redirectFromOmega(_extends({}, store.getState(), { history: history }));
      break;
    case EMBARK.ROUTE:
      // console.log(`Pantograph.graphite():${EMBARK.ROUTE}`, { type }) // eslint-disable-line
      redirectFromEmbark(_extends({}, store.getState(), { history: history }));
      break;
    case DEBARK.ROUTE:
      // console.log(`Pantograph.graphite():${DEBARK.ROUTE}`, { type }) // eslint-disable-line
      redirectFromDebark(_extends({}, store.getState(), { history: history }));
      break;
    case ALPHA.QUERY:
      // console.log(`Pantograph.graphite():${ALPHA.QUERY}`, { type }) // eslint-disable-line
      redirectFromAlpha(_extends({}, store.getState(), { history: history }));
      break;
  }
}
